...
use crate::app::models::user::{
    LoginResponse, LoginUserPayload, RegisterUserPayload, User, UserInfo,
};
...
impl UserService {
    ...
    pub async fn register_handler(
        &self,
        mut payload: RegisterUserPayload,
        ...
        payload.password = match &payload.password.is_empty() {
            ...
            false => hash_password(&payload.password)
                .unwrap()
                .as_str()
                .to_string(),
        };
        ...
        if self
            .repo
            .find_by_unique_identifier(&payload.username)
            .await?
            .is_some()
        {
        ...
        if self
            .repo
            .find_by_unique_identifier(&payload.email)
            .await?
            .is_some()
        {
    ...
    pub async fn login_handler(
        ...
        let token = create_token(user.id, &user.username, user.roles.clone(), config)
            .map_err(|_| AppError::InternalError(String::from("Create token failed")))?;
    ...
    pub fn gen_username(&self) -> Result<String, AppError> {
        let suffix = rand::rng()
